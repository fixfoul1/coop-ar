...
export function MazeRunner({ state, broadcast, pc }: Props) {
  ...
  return (
    <div className="flex w-full max-w-lg flex-col items-center gap-4">
      ...
      <div className={`w-full rounded-xl border px-6 py-3 text-center ${pc.isHost ? "border-violet-500/20 bg-violet-500/10" : "border-amber-500/20 bg-amber-500/10"}`}><p className={`text-sm ${pc.isHost ? "text-violet-300" : "text-amber-300"}`}>{pc.isHost ? "أنت ترى الخريطة — وجّه صديقك!" : "تحرك نحو النقطة الخضراء"}</p></div>
